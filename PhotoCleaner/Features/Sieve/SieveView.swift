...
/// Permite ao usuário escolher sua foto favorita em uma série de peneiras eliminatórias
struct SieveView: View {
    ...
    var body: some View {
        ZStack {
            ...
            case .fighting, .roundWon:
                sieveContent // Tela de peneira ativa
    ...
    private func setupTournament() {
        // Acessa explicitamente as fotos do grupo para forçar o SwiftData a carregar
        let photoCount = group.photos.count
        print("🎮 SieveView: setupTournament called, group has \(photoCount) photos")

        // Valida que há fotos suficientes (mínimo 2)
        guard photoCount >= 2 else {
            print("⚠️ SieveView: Not enough photos (\(photoCount))")
            errorMessage = "Este grupo precisa de pelo menos 2 fotos para iniciar uma peneira."
            showingError = true
            return
        }
        ...
        guard validPhotos.count >= 2 else {
            print("⚠️ SieveView: Not enough valid photos (\(validPhotos.count))")
            errorMessage = "As fotos do grupo não estão válidas. Tente executar uma nova análise."
            showingError = true
            return
        }

        print("🎮 SieveView: Starting setup with \(validPhotos.count) valid photos")
        viewModel.setupTournament(from: group)

        // Verifica se o setup funcionou corretamente
        if viewModel.photos.isEmpty {
            print("⚠️ SieveView: ViewModel photos is empty after setup")
            errorMessage = "Não foi possível carregar as fotos para esta peneira. Tente executar uma nova análise."
            showingError = true
        } else {
            print("✅ SieveView: Tournament setup successful with \(viewModel.photos.count) photos")
        }
    }
    ...
    private var startScreen: some View {
        VStack(spacing: 24) {
            ...
            Button {
                ...
                Label("Start Sieve", systemImage: "flag.checkered")
                    .font(.title2.bold())
    ...
    private var sieveContent: some View {
        HSplitView {
    ...
    private var sieveHeader: some View {
        HStack {
            Button {
                dismiss()
            } label: {
    ...
    private var sieveArena: some View {
    ...
    private func sieveCard(photo: PhotoAssetEntity, side: SieveViewModel.Side, isWinner: Bool, size: CGSize) -> some View {
        _ = PhotoAsset(from: photo)
        let cardWidth = max(0, (size.width - 80) / 2 - 32)
        let cardHeight = max(200, size.height - 200)
    ...
    private var actionButtons: some View {
        HStack(spacing: 40) {
            sieveButton(title: "Keep Left", side: .left, color: .blue)
            sieveButton(title: "Keep Right", side: .right, color: .purple)
        }
    }

    private func sieveButton(title: String, side: SieveViewModel.Side, color: Color) -> some View {
        Button {
        ...
        VStack(spacing: 24) {
            ...
            HStack(spacing: 20) {
                Button {
                    ...
                    Label("Sieve Again", systemImage: "arrow.counterclockwise")
    ...
    private var statsView: some View {
        HStack(spacing: 40) {
            statItem(value: "\(viewModel.photos.count)", label: "Photos Sieved")
...
#Preview {
    ...
    SieveView(group: PhotoGroupEntity(groupType: .similar))
}
