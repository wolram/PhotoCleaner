...
@MainActor
final class SieveViewModel: ObservableObject {
    // MARK: - Published State

    @Published var phase: SievePhase = .notStarted
    @Published var currentMatch: SieveMatch?
    ...
    func setupTournament(from group: PhotoGroupEntity) {
        ...
        guard sortedPhotos.count >= 2 else {
            print("⚠️ SieveViewModel: Not enough photos to start tournament (need at least 2, got \(sortedPhotos.count))")
            return
        }

        self.photos = sortedPhotos
        
        print("✅ SieveViewModel: Starting tournament with \(sortedPhotos.count) photos")
        ...
        guard !rounds.isEmpty, let firstMatch = rounds.first?.matches.first else {
            print("⚠️ SieveViewModel: Failed to create tournament rounds")
            return
        }
        ...
        print("✅ SieveViewModel: Tournament started with \(rounds.count) rounds")
    }

    private func buildBracket(from photoIds: [String]) {
        guard photoIds.count >= 2 else {
            print("⚠️ SieveViewModel: Cannot build bracket with less than 2 photos")
            return
        }
        ...
        print("🏆 SieveViewModel: Building bracket with \(photoIds.count) photos...")
        ...
        print("✅ SieveViewModel: Bracket built with \(rounds.count) rounds")
    }
        ...
        if currentRoundIndex < rounds.count {
            ...
            for matchIdx in 0..<rounds[currentRoundIndex].matches.count {
                if winnerIds.count >= 2 {
                    ...
                    rounds[currentRoundIndex].matches[matchIdx] = SieveMatch(
                        photo1Id: winner1,
                        photo2Id: winner2
                    )
                }
