...
@MainActor
final class ScanViewModelVision: ObservableObject {
    @Published var isLoadingAlbums = false
    @Published var albums: [PhotoLibraryServiceVision.Album] = []
    @Published var selectedAlbum: PhotoLibraryServiceVision.Album?
    @Published var lastScanResult: ScanResult?

    weak var appState: AppStateVision?
    ...
    func loadAlbums() async {
        ...
        albums = await PhotoLibraryServiceVision.shared.fetchAlbums()
    ...
    func startScan() async {
        ...
        let identifiers = await PhotoLibraryServiceVision.shared.fetchAllAssetIdentifiers(albumId: albumId)
        ...
        for group in duplicateGroups {
            if group.count > 1 {
                let assets = PhotoLibraryServiceVision.shared.fetchAssets(identifiers: Array(group.dropFirst()))
                for asset in assets {
                    totalSpaceRecoverable += PhotoLibraryServiceVision.shared.getFileSize(for: asset)
                }
            }
        }
